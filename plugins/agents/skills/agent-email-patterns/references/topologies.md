...
def make_inbox(username: str, client_id: str):
    return client.inboxes.create(username=username, client_id=client_id)
...
ROUTING = {
    "support": support.inbox_id,
    "sales": sales.inbox_id,
    "billing": billing.inbox_id,
}
...
# Router receives message.received webhooks (verified — see threat-model.md) and forwards
def on_message_received(event):
    msg = client.inboxes.messages.get(event["inbox_id"], event["data"]["message"]["message_id"])
...
```
    ...
    prospect@example.com  ->  sales@wzrd.tech
    vendor@example.com    ->  billing@wzrd.tech
```

Implementation: give each agent its own inbox and WebSocket listener. No router needed.

```bash
# One live event stream per agent; each process handles only its own inbox.
wzrdmail events tail --inbox-ids support@wzrd.tech --format json | ./handle_support
wzrdmail events tail --inbox-ids sales@wzrd.tech   --format json | ./handle_sales
wzrdmail events tail --inbox-ids billing@wzrd.tech --format json | ./handle_billing
```

Or register one webhook per inbox (`POST /v0/webhooks` with `inbox_id`) so each agent's endpoint only ever sees its own mail.
