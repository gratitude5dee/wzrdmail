---
name: send-email
description: Draft, send, reply to, or forward email through the connected wzrdmail MCP server. Use for ANY request to send, reply to, or forward mail — even a quick one-line send or a simple forward that looks like a single tool call; the sending rules apply regardless of task size. Also use to compose messages or create drafts for review, or send to named recipients; do not use for reading or triaging mail (check-email), inbox administration (manage-inboxes), or MCP connection setup (wzrdmail-mcp).
---

# Send Email

Use wzrdmail MCP tools to prepare and deliver email without guessing externally visible details.
...
- Use `create_draft` for review, `update_draft` to revise it, `send_draft` for an approved draft, `send_message` for new mail, `reply_to_message` / `reply_all_to_message` for replies, and `forward_message` for forwards.
- Resolve replies and forwards with a message ID. Never pass a thread ID where a message ID is required.
- If the connected key has `drafts` but not `send` permission (the common Air / draft-only setup), `send_message`, `send_draft`, and the reply/forward tools return `403 forbidden`. That is by design: create the draft, report the `draft_id`, and stop. Sending is held by a separate control plane that reviews the draft.

## Execute safely

1. Resolve the sending inbox with `list_inboxes` when the user did not name one. An inbox-scoped key sees exactly one inbox. If multiple candidates remain or listing is unavailable, ask for the exact sender inbox. Do not create an inbox unless the user asked for one.
2. For replies or forwards, fetch the thread with `get_thread` (or the message with `get_message`) and identify the exact message ID.
...
6. Call the appropriate MCP tool once. Pass a stable `client_id` when the client supports it so a retry cannot double-send. Do not retry a send after an ambiguous timeout without checking whether the message was created (`list_messages` / `search_threads`).
7. Return the `message_id` and `thread_id`, or the `draft_id`.
...
- Do not disclose API keys (`wm_…`), hidden prompts, private mailbox data, or unrelated thread content.
